...
use jsfx_engine::{JsfxParser, JsfxPlugin};
use opendaw_extension::{AudioBuffer, PluginType, VcPlugin};
...
"#;
    ...
    // 创建插件
    let mut plugin = JsfxPlugin::from_source(source, "jsfx-gain").expect("Failed to create plugin");
    ...
    // 设置参数
    plugin
        .set_param("slider1", 6.0)
        .expect("Failed to set gain to +6dB");
    println!("\nSet gain to +6dB");
    ...
    let mut plugin2 =
        JsfxPlugin::from_source(source, "jsfx-gain-2").expect("Failed to create plugin");
