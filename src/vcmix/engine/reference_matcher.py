...
class ReferenceMatcher:
    ...
    def compute_match(
        ...
        if needs_comp:
            ...
            summary_parts.append(
                f"Dynamics: mix needs {direction} compression (Δ{dr_delta:.1f} dB)"
            )
    ...
    def generate_adjustments(
        ...
        if diff.needs_eq:
            ...
            if mid_delta > _EQ_BAND_THRESHOLD:
                ...
                eq_params["peak_gain"] = -round(mid_delta * 20, 1)  # Scale ratio to dB
            ...
            if abs(diff.centroid_delta_hz) > _CENTROID_THRESHOLD_HZ:
                if diff.centroid_delta_hz < -_CENTROID_THRESHOLD_HZ:
                    # Mix is darker → boost high shelf
                    ...
                    # Mix is brighter → cut high shelf
        ...
        if diff.needs_comp:
            if diff.dr_delta_db > _DR_THRESHOLD_DB:
                # Mix is more dynamic → add compression
                comp_params = {
                    "threshold_db": -18,
                    "ratio": 2,
                    "attack_ms": 10,
                ...
                # Mix is less dynamic → reduce compression
                comp_params = {
                    "action": "reduce",
                    "ratio_adjust": -1,
                }
