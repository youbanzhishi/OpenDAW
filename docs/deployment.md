...
## 方式三：脚本一键部署（云电脑/国内环境推荐）

适用于云电脑或国内网络环境，项目 `scripts/` 下提供了一键脚本，内置踩坑记录和镜像配置：

```bash
git clone https://github.com/youbanzhishi/OpenDAW.git
cd OpenDAW

# 1. 安装Rust环境（系统依赖+国内镜像+内存自适应）
...
bash scripts/deploy-local.sh status  # 状态
bash scripts/deploy-local.sh logs    # 日志
bash scripts/deploy-local.sh stop    # 停止
```

| 脚本 | 用途 |
|------|------|
| `setup-rust.sh` | 安装Rust+系统依赖+国内镜像配置 |
| `cargo-mirror.sh` | 切换cargo镜像源（ustc/tuna/rsproxy/official） |
| `build.sh` | 编译构建，支持`--release`和`--bin`指定目标 |
| `deploy-local.sh` | 服务管理（start/stop/restart/status/logs） |
