...
use opendaw_extension::{
    AudioBuffer, ExtensionRegistry, HookContext, LocalBackend, ModelInput, ParamInfo, PluginType,
    ScriptEngine, ScriptValue, SimpleScriptEngine, VcPlugin,
};
...
impl VcPlugin for GainPlugin {
    ...
    fn init(
        ...
        sample_rate: f64,
        buffer_size: usize,
    ) -> Result<(), opendaw_extension::PluginError> {
    ...
    fn set_param(&mut self, id: &str, value: f64) -> Result<(), opendaw_extension::PluginError> {
        match id {
            ...
            _ => Err(opendaw_extension::PluginError::ParamNotFound(
                id.to_string(),
            )),
        }
    }
...
[Script] 执行赋值: volume = {}",
        ...
        let result = script
            .call_function("get_gain", &[ScriptValue::Float(0.8)])
            .unwrap();
